...
func CreateOrPatchSecurityContextConstraints(
	...
	if kerr.IsNotFound(err) {
		klog.V(3).Infof("Creating SecurityContextConstraints %s/%s.", meta.Namespace, meta.Name)
...
func PatchSecurityContextConstraintsObject(
	...
	klog.V(3).Infof("Patching SecurityContextConstraints %s with %s.", cur.Name, string(patch))
